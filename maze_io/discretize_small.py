"""Herramientas para transformar una imagen RGB en un grid discreto con celdas más pequeñas."""
...
def discretize_image(
    image: np.ndarray,
    tile_size: int = 1,  # Tamaño de tile más pequeño para mayor precisión
    ...
    for row in range(rows):
        ...
        for col in range(cols):
            ...
            # Ajuste de precisión para paredes finas
            # Si el color promedio dio FREE pero hay al menos un pixel de pared, marcar como WALL
            # Esto ayuda con resoluciones de tile > 1 cuando los trazos son finos
            if cell_type == CellType.FREE and tile_size > 1:
                min_color = tile.reshape(-1, 3).min(axis=0) # pixels más oscuros
                if _classify_color(min_color, colors, tolerance) == CellType.WALL:
            ...
            if cell_type == CellType.START:
                if start is None:
                    # Toleramos múltiples pixeles de inicio en este módulo fino agarrando el primero que aparezca
                    start = (row, col)
